...
try {
  ...
  console.error(`Could not prepare state directory: ${e.message}`);
}
...
if (fs.existsSync(STATE_FILE)) {
  try {
    ...
    if (!state.holdings || typeof state.holdings !== 'object') state.holdings = {};
    if (!state.trades || !Array.isArray(state.trades)) state.trades = [];
    if (!state.priceHistory || typeof state.priceHistory !== 'object') state.priceHistory = {};
    if (!state.dailySnapshots || !Array.isArray(state.dailySnapshots)) state.dailySnapshots = [];
...
// Serve dashboard. The source archive keeps the page beside server.js.
app.get('/', (req, res) => {
...
app.use((err, req, res, next) => {
  ...
  res.status(status).json({
    error: status === 400 ? 'Ogiltig begäran.' : 'Internt serverfel.'
  });
});
...
server.on('error', (err) => {
  console.error(`Servern kunde inte starta: ${err.message}`);
  process.exitCode = 1;
});
...
function shutdown(signal) {
  ...
  shuttingDown = true;
  log(`Servern stängs kontrollerat (${signal})`, 'system');
  saveState();
